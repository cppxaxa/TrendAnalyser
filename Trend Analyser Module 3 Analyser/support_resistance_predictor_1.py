...
if(len(sys.argv) < 4):
    print("python prog.py <directory> <filename> <segmentation_chart = y/n> <regression_chart = y/n>")
    exit()

directory = sys.argv[1]
filename = sys.argv[2]
enable_segmentation_chart = sys.argv[3]
enable_regression_chart = sys.argv[4]

if(enable_segmentation_chart == 'y'):
    enable_segmentation_chart = True
else:
    enable_segmentation_chart = False


if(enable_regression_chart == 'y'):
    enable_regression_chart = True
else:
    enable_regression_chart = False
...
try:
    names = filename.split(sep='_')
    print("#Symbol," + names[0])
except:
    print("#Symbol," + filename)

minDate = data.index[0];
maxDate = data.index[len(data.index) - 1]
scannedRange = maxDate - minDate
print("#Scanned," + str(scannedRange.days))

minSegmentDate = data.index[RsegmentedIndexes[i - 1]];
maxSegmentDate = data.index[RsegmentedIndexes[i]];
print("#SegmentDateMin," + minSegmentDate.strftime('%Y-%m-%d'))
print("#SegmentDateMax," + maxSegmentDate.strftime('%Y-%m-%d'))
print("#SegmentRange," + str((maxSegmentDate - minSegmentDate).days))
