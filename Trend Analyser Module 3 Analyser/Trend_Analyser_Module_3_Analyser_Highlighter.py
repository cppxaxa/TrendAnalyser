...
if(len(sys.argv) < 9):
    print("python program.py <directory> <filename> <syy> <smm> <sdd> <eyy> <emm> <edd>")
    exit()

directory = sys.argv[1]
filename = sys.argv[2]

sdate = sys.argv[5] + "/" +  sys.argv[4] + "/" + sys.argv[3]
edate = sys.argv[8] + "/" +  sys.argv[7] + "/" + sys.argv[6]
...
#fig, ax = plt.subplots()
#ax.plot_date(t, y, 'b-')
#ax1.axvspan(*mdates.datestr2num(['01/15/2015', '01/15/2016']), color='yellow', alpha=0.4)
